...
function AppContent() {
  ...
  useEffect(() => {
    if (!isReady || !tgUser) { setAppState('onboarding'); setIsLoading(false); return }
    const checkUser = async () => {
      setIsLoading(true)
      const supabase = createSupabaseClient(tgUser.id)
      const { data } = await supabase.rpc('get_my_profile', { _telegram_id: tgUser.id })
      if (!data || data.length===0) setAppState('onboarding')
      else {
        setProfile(data[0])
        if (data[0].approval_status==='approved') setAppState('approved')
        else if (data[0].approval_status==='pending') setAppState('pending')
        else setAppState('choose_role')
      }
      setIsLoading(false)
    }
    checkUser()
  ...
  if (!tgUser) return <div className="h-screen flex items-center justify-center">الرجاء فتح التطبيق داخل تيليجرام</div>
