...
export default function CustomerSignupPage() {
    ...
    const [form, setForm] = useState({ firstName: tgUser?.first_name || '', lastName: tgUser?.last_name || '', phone: '' })

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault(); if (!tgUser) return
        setLoading(true)
        const supabase = createSupabaseClient(tgUser.id)
        const { error } = await supabase.rpc('register_user', { _telegram_id: tgUser.id, _first_name: form.firstName, _last_name: form.lastName, _username: tgUser.username || null, _phone: form.phone, _role: 'customer' })
        setLoading(false)
