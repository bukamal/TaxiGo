...
export default function ProfilePage() {
    ...
    const toggleRole = async () => { const newRole = isDriverMode?'customer':'driver'; await supabase.from('profiles').update({ role:newRole }).eq('id',user?.id); setIsDriverMode(!isDriverMode) }
    ...
    return (
        <div className="p-4 space-y-4" dir={language==='ar'?'rtl':'ltr'}>
            ...
            <button onClick={()=>navigate('/admin')} className="w-full bg-gray-100 dark:bg-gray-800 py-4 rounded-2xl font-semibold">{t('profile.admin_panel')}</button>
