...
export default function RidePage() {
    ...
    useEffect(() => {
        if(!id||!user) return
        supabase.from('rides').select('*').eq('id',id).single().then(({data})=> { if(data) { setRide(data); const other = user.id===data.customer_id? data.driver_id : data.customer_id; if(other) supabase.from('profiles').select('*').eq('id',other).single().then(({data:p})=> { setOtherUser(p); if(p?.latitude) setDriverLocation([p.latitude, p.longitude]) }) } })
